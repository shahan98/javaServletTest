...
<table class="table">
    ...
    <tbody>
<%  for (ProductDetails pd : prodlist.getProduct(null,null,null)) {%>
      <tr>
        <td><%= pd.getId() %></td>
        <td><%= pd.getName() %></td>
        ...
        <td><%= pd.getRate() %></td>
        <td><a href="<%= pd.getImage() %>" ><IMG SRC="<%= pd.getImage() %>"  WIDTH="50" HEIGHT="50" /></a></td>
        <td><a href="LogServlet?log=deleteprod&prodid=<%=pd.getId()%>" class="btn btn-info" role="button">delete</a></td>
      </tr>
<% }%>
</tbody>
<a href="addproduct.jsp" class="btn btn-info" role="button">Add Product</a>
  </table>
</body>
</html>